...
### Script 3c: Analysing mother's data using NARMICE assuming exposure is MNAR
...
tipping_exposureMNAR <- as.data.frame(array(dim = c(dim = length(seq.int(-2, 0, by = 0.25)), 19))) # Number of sensitivity values we're going to try (varying the CSP from -2 to 0, in steps of 0.25), plus the number of parameters we're going to store (here, is 19 [see row below])
...
## Save this table
write_csv(tipping_exposureMNAR, file = "/user/home/ds16565/BloodDonation/Results_mum/Results_mum_NARMICE_exposureMNAR_noM.csv")
